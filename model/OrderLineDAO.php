...
class OrderLineDAO extends AbstractClient {
	public function getOrderLinesByOrder($order_id){
		$result=array();
		$req = $this->pdo_db->prepare("SELECT * FROM ".$this->tb_prefix."privateorder_line WHERE fk_order=".$order_id);
		$req->execute();
	...
	public function createOrderLine(OrderLine $orderline){
		 $req=$this->pdo_db->prepare("INSERT INTO ".$this->tb_prefix."privateorder_line SET fk_order=:fk_order, fk_product=:fk_product, amount=:amount, ol_value=:value");
